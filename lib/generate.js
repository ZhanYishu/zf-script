...
function generateFiles (templatePath, dir, meta) {
  const filesPath = path.join(templatePath, './template')
...
function copyFile (file, target) {
  ...
  .then(() => fs.copy(file, target))
}

function makeFile (file, target, meta) {
  return new Promise((resolve, reject) => {
    const template = artTemplate(file, meta)
...
module.exports = function generate (template, dir, meta) {
  ...
  .resolve()
  .then(() => {
    spinner = ora(`Fetching template from ${templatesRepo}`).start()
  })
